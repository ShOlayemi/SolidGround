...
import { partnerLabel, relationshipLabel } from "@/lib/mode";
  ...
  try {
    ...
    return <div className="max-w-[1200px]">
      <header className="mb-8 flex items-center gap-4">
        ...
        <div><p className="text-sm font-medium text-accent-600">SolidGround AI</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-text-primary">Welcome back, {name}</h1><p className="mt-2 text-text-secondary">Your {rLabel} intelligence, at a glance.</p></div>
      </header>
      <div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">
        ...
        <Link href="/dashboard/pairings"><StatCard variant="metric" value={pairings.pairings?.length ?? 0} label={pairings.pairings?.length ? `${pLabel.charAt(0).toUpperCase() + pLabel.slice(1)} alignments` : `Invite your ${pLabel} →`} /></Link>
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-xl border border-card-border bg-card-bg p-6"><SectionHeader title="Quick Actions" /><div className="mt-5 grid gap-3 sm:grid-cols-2">{[[ClipboardCheck,"Take Assessment","/dashboard/blueprint"],[BarChart3,"View Results","/dashboard/scores"],[UserPlus,`Invite ${pLabel.charAt(0).toUpperCase() + pLabel.slice(1)}`,"/dashboard/pairings"],[FileText,"View Report","/dashboard/reports"]].map(([Icon,label,href]) => { const C = Icon as typeof ClipboardCheck; return <Link key={String(label)} href={String(href)} className="flex items-center gap-3 rounded-lg border border-card-border p-4 text-sm font-medium text-text-primary transition hover:border-accent-300 hover:bg-card-hover"><C size={18} className="text-accent-600" />{String(label)}</Link> })}</div></section>
      </div>
    </div>;
