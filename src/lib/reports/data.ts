...
import type { AlignmentResults, ComparisonReport, RelationshipType } from "@/types";
...
export async function reportContext() {
  ...
  const { data: session } = await supabase.from("assessment_sessions").select("id, status, mode").eq("user_id", user.id).eq("status", "completed").order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (!session) return { supabase, userId: user.id, name: profile?.display_name ?? profile?.full_name ?? "Your profile", results: null, mode: undefined as RelationshipType | undefined };
  ...
  return { supabase, userId: user.id, name: profile?.display_name ?? profile?.full_name ?? "Your profile", sessionId: session.id, results, mode: session.mode as RelationshipType | undefined };
}
export async function comparisonContext(pairingId: string) {
  const ctx = await reportContext();
  if (!ctx) return null;
  const { data: pairing } = await ctx.supabase.from("pairings").select("id, inviter_user_id, invitee_user_id, inviter_session_id, status, relationship_type, alignment_results").eq("id", pairingId).or(`inviter_user_id.eq.${ctx.userId},invitee_user_id.eq.${ctx.userId}`).maybeSingle();
  ...
  return { ...ctx, pairing: { ...pairing, inviterName: nameOf(pairing.inviter_user_id), inviteeName: nameOf(pairing.invitee_user_id), alignment: pairing.alignment_results as AlignmentResults, report }, mode: (pairing.relationship_type as RelationshipType) ?? ctx.mode };
}
