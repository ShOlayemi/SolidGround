...
const MODE_OPTIONS: { value: RelationshipType; label: string }[] = [
...
export function DiscoverClient({ initialUsers }: { initialUsers: DiscoverUser[] }) {
  ...
  async function connect(id: string) { setBusy(id); setError(null); const r = await sendConnectionRequest(id, mode); if (r.success) setUsers((v) => v.map((u) => u.id === id ? { ...u, hasPending: true } : u)); else setError(r.error ?? "Could not send request."); setBusy(null); }
  ...
  return <div className="max-w-[960px] mx-auto py-8 md:py-10">
    ...
    <div className="mb-6 flex flex-wrap items-center gap-4">
      <div className="relative flex-1 min-w-[200px]">
        <Search className="absolute left-4 top-3.5 text-solid-text-tertiary" size={18}/>
        <input value={q} onChange={(e) => void search(e.target.value)} placeholder="Search by name" className="w-full rounded-xl border border-solid-border bg-solid-surface py-3 pl-11 pr-4 text-[14px] outline-none focus:border-accent-400" aria-label="Search users"/>
      </div>
      <fieldset className="flex items-center gap-2 shrink-0">
        <legend className="text-[13px] font-medium text-solid-text-secondary">I&apos;m looking for:</legend>
        <div className="flex rounded-lg border border-solid-border bg-solid-surface p-0.5">
            ...
            <button
              ...
              className={`px-3 py-1.5 text-[13px] rounded-md font-medium transition-colors ${
                mode === opt.value
                  ...
                  : "text-solid-text-secondary hover:text-solid-text"
              }`}
            >
              {opt.label}
            </button>
